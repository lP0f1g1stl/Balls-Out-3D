...
- (ALSdk *)initializeSdkWithMetaData:(NSString *)serializedMetaData andCompletionHandler:(ALSdkInitializationCompletionHandler)completionHandler
{
    NSDictionary *infoDict = [[NSBundle mainBundle] infoDictionary];
    NSString *sdkKey = infoDict[@"AppLovinSdkKey"];
    self.sdk = [ALSdk sharedWithKey: sdkKey settings: [self generateSDKSettingsForMetaData: serializedMetaData]];
...
- (void)positionAdViewForAdUnitIdentifier:(NSString *)adUnitIdentifier adFormat:(MAAdFormat *)adFormat
{
    ...
    else if ( [adViewPosition isEqual: @"CenterLeft"] || [adViewPosition isEqual: @"CenterRight"] )
    {
        if ( MAAdFormat.mrec == adFormat )
        {
            [constraints addObject: [adView.widthAnchor constraintEqualToConstant: adViewSize.width]];
            
            if ( [adViewPosition isEqual: @"CenterLeft"] )
            {
...
- (ALSdkSettings *)generateSDKSettingsForMetaData:(NSString *)serializedMetaData
{
